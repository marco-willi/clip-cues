...
# HuggingFace dataset ids for the published datasets (used when no local copy exists,
# e.g. on a fresh Lambda box where data/ is not rsynced).
HF_DATASET_IDS = {
...
def load_split_dataset(dataset_name: str, dataset_path: Path):
    ...
    The local `data/datasets/<name>` copy is excluded from the Lambda rsync, so remote
    runs fall back to pulling the published dataset from the Hub.
    """
...
def extract_embeddings(dataset, output_path: Path, identifier: str, device: str = "cuda"):
    """Extract CLIP embeddings for an (already-loaded) dataset.
    ...
    extractor = CLIPLargePatch14(cache_dir="data/hf_cache")
    extractor.freeze()
...
Examples:
  ...
  python extract_clip_embeddings.py synthclic --device cpu
    ...
    # Extract embeddings
    extract_embeddings(dataset, output_path, identifier, device=args.device)
