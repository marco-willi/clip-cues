...
def create_text_embeddings(
    vocab_csv_path: Path, output_path: Path, device: str = "cuda", cache_dir: str = "data/hf_cache"
):
    ...
    with torch.no_grad():
        for prompt in tqdm(prompts, desc="Extracting text embeddings"):
            ...
            # transformers <5 returns the projected text features as a tensor; transformers >=5
            # returns the text-model output object, so we project the pooled token ourselves to
            # land in the shared CLIP space (matches the legacy behavior, 768-d for ViT-L/14).
            if isinstance(out, torch.Tensor):
                text_features = out
            else:
                text_features = model.text_projection(out.pooler_output)
