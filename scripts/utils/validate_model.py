...
def run_inference(model, dataset_split, transforms, device: torch.device, batch_size: int = 32):
    ...
    for i in tqdm(range(0, len(dataset_split), batch_size)):
        ...
        batch_ids = (
            batch["image_id"] if isinstance(batch["image_id"], list) else [batch["image_id"]]
        )
...
def calculate_per_source_metrics(predictions, ground_truth, sources, dataset_name="unknown"):
    ...
    real_sources = [
        src for src, data in source_data.items() if all(label == 0 for label in data["labels"])
    ]

    # Find synthetic sources (label=1)
    synthetic_sources = [
        src for src, data in source_data.items() if all(label == 1 for label in data["labels"])
    ]
...
def print_results(overall_metrics, per_source_metrics, mAP, real_sources, synthetic_sources):
    ...
    for source, metrics in sorted_sources:
        print(
            f"{source:<30} {metrics['n_real']:8d} {metrics['n_synthetic']:8d} "
...
def main():
    parser = argparse.ArgumentParser(description="Validate a model on a dataset test split")
    parser.add_argument(
        "checkpoint",
        type=Path,
